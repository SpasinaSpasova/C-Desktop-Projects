...
namespace film
{
    public partial class PropertiesForm : DevExpress.XtraBars.Ribbon.RibbonForm
    {
            ...
            if (result == DialogResult.Yes)
            {
                using (var db = new MovieLottoDBEntities())
                {
                    ...
                    if (currentFilm != null)
                    {
                        ...
                        var lotteryRow = db.LotaryRoles.FirstOrDefault(x => x.FilmId == film.Id);
                        if (lotteryRow == null)
                        {
                        ...
                        {
                            MessageBox.Show("This film can not be delete!", "Delete", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
        ...
        private void FilmGridView_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            if (e.CellValue != null && (e.CellValue.ToString().StartsWith("https:")|| e.CellValue.ToString().StartsWith("http:")))
            {
                Uri uri = new Uri(e.CellValue.ToString());
